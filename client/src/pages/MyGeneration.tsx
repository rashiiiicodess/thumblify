            ...
            {thumbnails.map((thumb: IThumbnail)=>{
                ...
                return (
                  <div key={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border-white/10 transition shadow-xl break-inside-avoid">
                    ...
                    {thumb.isGenerating && <div className="absolute inset-0 bg-black/50 flex items-center text-sm font-medium text-white">Generating...</div>}
                    </div>
                    ...
                    <div className="p-4 space-y-2">
                      ...
                      <p className="text-sm text-zinc-500">{new Date(thumb.createdAt!).toDateString()}</p>
                    </div>
